...
the same name or a strata with more thn one level in each equation.
...
Even for the grous of equations we have to provide (distinct) names
...
At the same time we can considered for the evaluation only eqautions
which assume particoular level of some strata. In this case if 
we ask for a statum to assume a level it does not have nothing will 
be computed (anyway the resulting object still a data frame of zero
rows, for consistency of the outut class). On the other hand if we
